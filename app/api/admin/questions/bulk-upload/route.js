...
export async function POST(request) {
  try {
    ...
    const isOverallUpload = sectionId === "overall_section"

    console.log("Bulk upload request received:", {
      category, examId, sectionId, examName, sectionName,
    ...
    // For non-overall uploads, validate the specific section exists
    if (!isOverallUpload) {
    ...
    // Parse questions data (file or text)
    let questionsData = []
          ...
          .map((line) => {
            ...
            headers.forEach((header, headerIndex) => {
              ...
              if (header === "options" && value) {
                question[header] = value.split("|").map((opt) => opt.trim()).filter(Boolean)
              } else if (header === "tags" && value) {
                question[header] = value.split("|").map((tag) => tag.trim()).filter(Boolean)
              } else if (header === "marks" || header === "negativeMarks") {
                question[header] = parseFloat(value) || (header === "marks" ? 1 : 0.25)
              } else {
    ...
    // For overall upload, validate every question has a valid sectionName
    if (isOverallUpload) {
      const availableSectionNames = exam.sections.map((s) => s.name?.toLowerCase().trim())
      const invalidQuestions = questionsData
        .map((q, i) => ({ q, i }))
        .filter(({ q }) => {
          if (!q.sectionName || !q.sectionName.trim()) return true
          return !availableSectionNames.includes(q.sectionName.toLowerCase().trim())
        })

      if (invalidQuestions.length > 0) {
        const details = invalidQuestions.map(
          ({ i, q }) =>
            `Question ${i + 1}: sectionName "${q.sectionName || "(missing)"}" does not match any exam section. Available: [${exam.sections.map((s) => s.name).join(", ")}]`
        )
        return NextResponse.json(
          {
            message: "Some questions have missing or invalid sectionName for Overall upload",
    ...
    if (exam.totalQuestions + questionsData.length > exam.maxQuestions) {
      return NextResponse.json(
        { message: `Exam has reached its maximum question limit of ${exam.maxQuestions}` },
        { status: 400 }
      )
    }
    ...
    for (let i = 0; i < questionsData.length; i++) {
      try {
        // Resolve the actual sectionId and sectionName for this question
        let resolvedSectionId = sectionId
        let resolvedSectionName = sectionName

        if (isOverallUpload) {
          // Match section by name (case-insensitive)
          const matchedSection = exam.sections.find(
            (s) => s.name?.toLowerCase().trim() === questionsData[i].sectionName?.toLowerCase().trim()
          )

          if (!matchedSection) {
            errors.push(
              `Question ${i + 1}: Could not find section "${questionsData[i].sectionName}" in exam`
            )
            continue
          }

          resolvedSectionId = matchedSection.sectionId
          resolvedSectionName = matchedSection.name
        }
        ...
        const questionData = {
          ...
          category,
          examName,
          // Always use the resolved section name (not what came from the dropdown)
        ...
        //  return NextResponse.json( { questions : questionsData, status: 201 })

        const question = new Question(questionData)
        await question.save()

        // Add question ID to the correct section in the exam
        await Exam.findByIdAndUpdate(
          ...
          {
            arrayFilters: [{ "section.sectionId": resolvedSectionId }],
            new: true,
          }
        )
    ...
    return NextResponse.json(
      { message: "Internal server error during bulk upload", error: error.message },
      { status: 500 }
    )
  }
}
