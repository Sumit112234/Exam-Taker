...
export async function POST(request, { params }) {
  try {
    ...
    // Delete existing result if present (and revert its contribution to exam statistics)
    const existingResult = await Result.findOne({ exam: exam._id, user: user.userId })
    if (existingResult) {
      // Revert the previous result's contribution to exam statistics
      // if (exam.statistics.totalAttempts > 1) {
      //   const prevTotal = exam.statistics.totalAttempts
      //   exam.statistics.averageScore =
      //     (exam.statistics.averageScore * prevTotal - existingResult.score) / (prevTotal - 1)
      //   exam.statistics.passRate =
      //     (exam.statistics.passRate * prevTotal - (existingResult.isPassed ? 100 : 0)) / (prevTotal - 1)
      //   exam.statistics.totalAttempts -= 1
      // } else {
      //   // This was the only attempt, reset stats to zero
    ...
    // Update exam statistics with new result
    exam.statistics.totalAttempts += 1
    exam.statistics.averageScore =
    ...
    return NextResponse.json({
      message: existingResult
        ? "Previous result replaced successfully"
